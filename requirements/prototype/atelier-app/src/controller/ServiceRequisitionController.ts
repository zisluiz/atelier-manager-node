...
export default class ServiceRequisitionController {
    ...
    public getExecutionService(service:Service) {
        ...
        let executionService = new ExecutionService(service, 20.0, instancedCloths, spendedTimes, [], [], this.statusList[0])

        service.clothes.forEach( (cloth: Cloth) => {
            let clothInstancedClothes: ClothInstance[] = [];
            ...
            for (let index = 0; index < cloth.quantity; index++) {
                ...
                clothInstancedClothes.push(instancedCloth)
                generatedId++;
            }

            executionService.createDefaultSpendedTime(cloth, clothInstancedClothes, spendedTimes);
        });
